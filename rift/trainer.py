...
from .utils.vae_latent_cache import VaeCacheCollator, VaeLatentCache

logger = get_logger(__name__)


class Wan22Trainer:
    def __init__(self, model, train_dataset, *, cfg: DictConfig):
        self.model = model
        self.train_dataset = train_dataset
        self.cfg = cfg
        self.vae_cache_dir = cfg.get("vae_cache_dir", None)
        dataset_vae_namespace = getattr(train_dataset, "vae_cache_namespace", None)
        if dataset_vae_namespace is not None and not self.vae_cache_dir:
        ...
        if (getattr(self.model, "compile_training_denoise", False)
                or (getattr(self.model, "vae_encode_mode", "sequential") == "cudagraphs"
                    and not self.vae_cache_dir)):
            self._warmup_compiled_training()
        ...
        if hasattr(unwrapped_model, "_anticip_step"):
            unwrapped_model._anticip_step = int(self.global_step)
            logger.info("Restored model curriculum step to %d.", self.global_step)

        # Revalidate against restored weights, including after a compile warmup.
        self._configure_vae_cache(unwrapped_model)

        logger.info("Train dataset size: %d", len(self.train_dataset))

    def _configure_vae_cache(self, model):
        if not self.vae_cache_dir:
            return
        namespace = getattr(self.train_dataset, "vae_cache_namespace", None)
        with self.accelerator.autocast():
            model._vae_latent_cache = VaeLatentCache(
                self.vae_cache_dir, model.vae, model.torch_dtype, model.device,
                precomputed_namespace=namespace,
            )

    def _warmup_compiled_training(self):
        """Warm enabled VAE/MoT paths before ZeRO without advancing training."""
        logger.info("Prewarming compiled training paths before DeepSpeed initialization.")
        start = time.perf_counter()
        model = self.model
        compile_mot = getattr(model, "compile_training_denoise", False)
        curriculum_step = getattr(model, "_anticip_step", None)
        previous_cache = getattr(model, "_vae_latent_cache", None)
        python_rng, numpy_rng = random.getstate(), np.random.get_state()
        device = self.accelerator.device
        cuda_devices = [device] if device.type == "cuda" else []
        try:
            with torch.random.fork_rng(devices=cuda_devices):
                self._configure_vae_cache(model)
                # Keep worker RNGs and the production loader iterator untouched.
                loader = DataLoader(
                    self.train_dataset,
                ...
                with self.accelerator.autocast():
                    if (getattr(model, "vae_encode_mode", "sequential") == "cudagraphs"
                            and not self.vae_cache_dir):
                        # The CUDA Graph backend warms up first and captures on
                        # a later call. Do both before the first training update.
            ...
            if curriculum_step is not None:
                model._anticip_step = curriculum_step
            if previous_cache is not None:
                model._vae_latent_cache = previous_cache
            elif hasattr(model, "_vae_latent_cache"):
                del model._vae_latent_cache
            random.setstate(python_rng)
            np.random.set_state(numpy_rng)
        logger.info("Finished compile warmup in %.2f seconds.", time.perf_counter() - start)
    ...
    def _build_loader(self, dataset, worker_init_fn=None):
        ...
        return DataLoader(
            ...
            collate_fn=VaeCacheCollator() if self.vae_cache_dir else None,
        )
    ...
    def _resume_training_state(self):
        ...
        if not resume_path.exists():
            raise FileNotFoundError(f"Training-state directory not found: {resume}")
        if not resume_path.is_dir():
            raise ValueError(
                "`resume` must point to an Accelerate training-state directory. "
                "Weights-only training initialization is not supported; set "
                "`resume=null` to start a new run from the configured base artifacts."
            )
