...
from .helpers.static_tensor_cache import cached_tensor, exact_device_key
from .helpers.vae_encoding import OnlineVaeEncoder
from .mot import MoT
...
class FastWAM(torch.nn.Module):
    ...
    def __init__(
        ...
        if vae_encode_mode not in {"sequential", "batched", "cudagraphs"}:
            raise ValueError("`vae_encode_mode` must be sequential, batched, or cudagraphs.")
        if vae_encode_batch_size is not None and (
            type(vae_encode_batch_size) is not int or vae_encode_batch_size < 1
        ):
            raise ValueError("`vae_encode_batch_size` must be a positive integer or null.")
    ...
    def _encode_video_latents(self, video_tensor, tiled=False, tile_size=(30, 52), tile_stride=(15, 26)):
        if self.vae_encode_mode != "sequential":
            if tiled:
                raise NotImplementedError("Batched online VAE encoding does not support tiled encoding.")
            if not hasattr(self, "_online_vae_encoder"):
                self._online_vae_encoder = OnlineVaeEncoder(
                    self.vae,
                    cudagraphs=self.vae_encode_mode == "cudagraphs",
                    batch_size=self.vae_encode_batch_size,
                )
            return self._online_vae_encoder(video_tensor.to(device=self.device))
        z = self.vae.encode(
            video_tensor,
            device=self.device,
            tiled=tiled,
    ...
    def build_inputs(self, sample, tiled: bool = False):
        indexed_latents = "video_latents" in sample
        if indexed_latents:
            if "video" in sample:
                raise ValueError("Training samples cannot contain both pixels and indexed VAE latents.")
            video_shape = sample.get("video_shape")
            if (not isinstance(video_shape, torch.Tensor) or video_shape.ndim != 2
                    or video_shape.shape[1] != 4 or video_shape.shape[0] == 0
                    or video_shape.dtype not in (torch.int32, torch.int64)):
                raise ValueError("Indexed VAE samples require integer `video_shape` rows [3,T,H,W].")
            shapes = video_shape.tolist()
            if any(shape != shapes[0] for shape in shapes):
                raise ValueError("Indexed VAE samples must have identical original video shapes.")
            batch_size = len(shapes)
            channels, num_frames, height, width = shapes[0]
            if channels != 3 or min(num_frames, height, width) <= 0:
                raise ValueError("Indexed VAE samples require positive video dimensions and 3 channels.")
        else:
            video = sample["video"]
            if video.ndim != 5:
        ...
        if action.shape[0] != batch_size:
            raise ValueError("Action batch size does not match the video batch size.")
        ...
        latent_cache = getattr(self, "_vae_latent_cache", None)
        if indexed_latents:
            if latent_cache is None:
                raise RuntimeError("Indexed VAE latents require a configured VAE cache identity.")
            input_latents = latent_cache.accept_precomputed(
                sample["video_latents"], sample["vae_cache_keys"], sample["vae_cache_namespace"],
                (batch_size, 3, num_frames, height, width), tiled=tiled,
            )
        else:
            ...
            if latent_cache is None:
                ...
                input_latents = latent_cache.encode(input_video, sample["vae_cache_keys"], tiled=tiled)
    ...
    def _build_mot_attention_mask(
        ...
        key = (video_seq_len, action_seq_len, video_tokens_per_frame,
               getattr(self.video_expert, "video_attention_mask_mode", None), exact_device_key(device))
        return cached_tensor(self, "first_frame_attention_mask", key, build)
