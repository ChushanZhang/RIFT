...
class MoT(nn.Module):
    ...
    def _forward_joint_layer(
        ...
        """FastWAM's tensor layer interface with RIFT's checkpoint boundaries."""
        (
            q_video, k_video, v_video, residual_video,
            gate_msa_video, shift_mlp_video, scale_mlp_video, gate_mlp_video,
            video_checkpointing,
        ) = self._build_expert_attention_io(
            expert=self.mixtures["video"], block=video_block, x=video_tokens,
            freqs=video_freqs, t_mod=video_t_mod,
        )
        (
            q_action, k_action, v_action, residual_action,
            gate_msa_action, shift_mlp_action, scale_mlp_action, gate_mlp_action,
            action_checkpointing,
        ) = self._build_expert_attention_io(
            expert=self.mixtures["action"], block=action_block, x=action_tokens,
        ...
        video_tokens = self._apply_post_with_optional_checkpoint(
            block=video_block,
            ...
            use_gradient_checkpointing=video_checkpointing,
            mixed_slice=mixed[:, :video_seq_len],
            context_payload={"context": video_context, "mask": video_context_mask},
        )
        action_tokens = self._apply_post_with_optional_checkpoint(
            block=action_block,
            ...
            use_gradient_checkpointing=action_checkpointing,
            mixed_slice=mixed[:, video_seq_len:],
    ...
    def _forward_compiled(
        self, embeds_all, attention_mask, freqs_all, context_all, t_mod_all,
    ):
        if self.expert_order != ["video", "action"]:
            raise ValueError("Compiled training requires experts ordered as ['video', 'action'].")
        video_tokens, action_tokens = embeds_all["video"], embeds_all["action"]
        if attention_mask.shape[0] != video_tokens.shape[1] + action_tokens.shape[1]:
            raise ValueError("Attention mask seq length mismatch with video/action tokens.")
        if not hasattr(self, "_compiled_joint_layer"):
            self._compiled_joint_layer = torch.compile(self._forward_joint_layer, fullgraph=True)

        video_context = context_all.get("video") or {}
        action_context = context_all.get("action") or {}
        for layer_idx in range(self.num_layers):
            video_tokens, action_tokens = self._compiled_joint_layer(
                ...
                video_context=video_context.get("context"),
                video_context_mask=video_context.get("mask"),
                action_context=action_context.get("context"),
                action_context_mask=action_context.get("mask"),
                attention_mask=attention_mask,
            )
    ...
    def forward(
        ...
        if self.training and self.compile_training_layers:
            return self._forward_compiled(
                embeds_all, attention_mask, freqs_all, context_all, t_mod_all,
            )
