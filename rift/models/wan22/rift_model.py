...
class RIFTModel(FastWAM):
    ...
    def build_anticip(
        ...
        fuse_training_paths: bool = False,
        ...
        anticipation = AnticipModule(future_tokens, hidden_dim, patch_dim)
        if not train_probe_head:
            # Consume the original CPU initialization RNG, but never move the
            # disabled probe to the GPU or include it in the optimizer/state dict.
            anticipation.head = None
        anticipation = anticipation.to(
            device=self.device,
        ...
        self.fuse_training_paths = bool(fuse_training_paths)
    ...
    def _build_anticip_attention_mask(
        ...
        return cached_tensor(self, "anticip_attention_mask", key, build)
    ...
    def _training_mot_forward(self, video_pre, action_pre, attention_mask):
        pre = {"video": video_pre, "action": action_pre}
        return self.mot(
            embeds_all={name: state["tokens"] for name, state in pre.items()},
            attention_mask=attention_mask,
            freqs_all={name: state["freqs"] for name, state in pre.items()},
            context_all={
                name: {"context": state["context"], "mask": state["context_mask"]}
                for name, state in pre.items()
            },
            t_mod_all={name: state["t_mod"] for name, state in pre.items()},
        )

    def _fused_training_mot_forward(
        self, video_pre_a, action_pre_a, video_pre_b, action_pre_b, mask_b
    ):
        def merge(a, b):
            # RoPE has no batch axis; equal grids use the same positional frequencies.
            return {
                "freqs": a["freqs"],
                **{
                    key: torch.cat([a[key], b[key]], dim=0)
                    for key in ("tokens", "context", "context_mask", "t_mod")
                },
            }

        # Both paths forbid video -> action attention. The B mask changes only
        # A-action outputs, which have no loss; all supervised outputs stay the same.
        output = self._training_mot_forward(
            merge(video_pre_a, video_pre_b), merge(action_pre_a, action_pre_b), mask_b
        )
        batch_size = video_pre_a["tokens"].shape[0]
        return (
            {name: tokens[:batch_size] for name, tokens in output.items()},
            {name: tokens[batch_size:] for name, tokens in output.items()},
        )
    ...
    def training_loss(self, sample, tiled: bool = False):
        ...
        # Longer Path A clips retain their complete video objective via two forwards.
        fuse_paths = self.fuse_training_paths and input_latents.shape[2] == self.anticip_latent_t
        ...
        if not fuse_paths:
            ...
            output_a = self._training_mot_forward(video_pre_a, action_pre_a, mask_a)
        ...
        video_pre_b = {**video_pre_b, "tokens": video_tokens_b}
        if fuse_paths:
            output_a, output_b = self._fused_training_mot_forward(
                video_pre_a, action_pre_a, video_pre_b, action_pre_b, mask_b
            )
        else:
            output_b = self._training_mot_forward(video_pre_b, action_pre_b, mask_b)
        ...
        if self.mot.anticip.head is not None:
            # This diagnostic probe cannot shape future_states: its input is detached.
        ...
        loss_anticip = loss_fm.new_zeros(())
        if self.mot.anticip.head is not None:
