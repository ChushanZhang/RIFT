...
from ..transforms.image import ToTensor
from ..utils.normalizer import LinearNormalizer, NormMode
from rift.utils.pytorch_utils import dict_apply
from rift.utils.logging_config import get_logger
from .base_processor import BaseProcessor

logger = get_logger(__name__)

class RIFTProcessor(BaseProcessor):
    def __init__(
        self,
        # keys
        ...
        self.image_sample_indices = None
    ...
    def set_image_sample_indices(self, indices):
        """Select image frames before per-frame transforms; retain full state/padding."""
        indices = tuple(indices)
        if (not indices or any(type(index) is not int for index in indices)
                or tuple(sorted(set(indices))) != indices
                or indices[0] < 0 or indices[-1] >= self.num_obs_steps):
            raise ValueError("Image sample indices must be sorted unique observation indices.")
        self.validate_deterministic_image_transforms()
        self.image_sample_indices = indices

    def validate_deterministic_image_transforms(self):
        """Reject transforms whose effects cannot be reused from a latent cache."""
        for transforms in (self.train_transforms, self.val_transforms):
            if transforms is None:
                continue
            pipelines = transforms.values() if isinstance(transforms, dict) else [transforms]
            for pipeline in pipelines:
                if any(type(transform) not in (ToTensor, Resize) for transform in pipeline):
                    raise ValueError("Early sampling and indexed latents require only ToTensor and Resize transforms.")
    ...
    def preprocess(self, data: Dict[str, Any], *, process_images: bool = True) -> Dict[str, Any]:
        """
        ...
        if process_images:
            ...
            for meta in self.shape_meta["images"]:
                ...
                if self.image_sample_indices is not None:
                    assert image.shape[0] == self.num_obs_steps, \
                        f"Expected {self.num_obs_steps} input image frames, got {image.shape[0]}"
                    image = image[list(self.image_sample_indices)]
                ...
                image_steps = (self.num_obs_steps if self.image_sample_indices is None
                               else len(self.image_sample_indices))
                meta_shape = [image_steps] + shape
