...
class RIFTConfigTest(unittest.TestCase):
    def test_compilation_and_fusion_can_be_enabled_together(self) -> None:
        with initialize_config_dir(version_base=None, config_dir=str(CONFIG_DIR)):
            cfg = compose(config_name="train", overrides=[
                ...
                "model.vae_encode_batch_size=4",
            ])
            OmegaConf.resolve(cfg)
        self.assertTrue(cfg.model.compile_training_denoise)
        self.assertTrue(cfg.model.fuse_training_paths)
        self.assertEqual(cfg.model.vae_encode_mode, "cudagraphs")
        self.assertEqual(cfg.model.vae_encode_batch_size, 4)
        self.assertTrue(cfg.model.mot_checkpoint_mixed_attn)
        self.assertTrue(cfg.model.video_dit_config.use_gradient_checkpointing)
        self.assertTrue(cfg.model.action_dit_config.use_gradient_checkpointing)
        ...
        with initialize_config_dir(version_base=None, config_dir=str(CONFIG_DIR)):
            for task, expected in tasks.items():
                with self.subTest(task=task):
                    ...
                    self.assertFalse(model.compile_training_denoise)
                    self.assertEqual(model.vae_encode_mode, "sequential")
                    self.assertIsNone(model.vae_encode_batch_size)
